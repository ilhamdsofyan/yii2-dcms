...
class AuditTrailHelper
{
    ...
    public function leaveTrailForCreateRoute($model,$post)
    {
        ...
        $event->leaveTrail('CREATE', null, null, null, $post, 'docotel\dcms\models\route');
        
        $event->leaveTrail('SET','name' ,$post , null, $post, 'docotel\dcms\models\route');
    ...
    public function leaveTrailForRemoveRoute($model,$routes)
    {
        $event = new LoggableBehavior();
        foreach ($routes as $r) {
            $event->leaveTrail('DELETE', null, null, null, $r, 'docotel\dcms\models\route');
        }
    }
    ...
    public function leaveTrailForAssignAssignment($items, $id)
    {
        ...
        $event = new LoggableBehavior();
        foreach ($items as $r ) {
            $event->leaveTrail('CREATE', null, null, null, $id, 'docotel\dcms\models\assignment');
            $event->leaveTrail('SET','item_name' ,$r , null, $id, 'docotel\dcms\models\assignment');
            $event->leaveTrail('SET','user_id' ,$id , null, $id, 'docotel\dcms\models\assignment');
        }
    }
     ...
     public function leaveTrailForRemoveAssignment($items, $id)
    {
        $event = new LoggableBehavior();
        foreach ($items as $r ) {
            $event->leaveTrail('DELETE', null, null, null, $id, 'docotel\dcms\models\assignment');
        }
    }
