<?php
namespace docotel\dcms\behaviors;

use Yii;
use yii\base\Behavior;
use yii\db\ActiveRecord;
use docotel\dcms\models\AuditTrail as AuditTrailModel;
use Exception;
use docotel\dcms\models\AuditTrailDetail;
...
class LoggableBehavior extends Behavior
{
    ...
    {
        if(($id != null) && 
            ($model == "docotel\dcms\models\AuthItem" || "docotel\dcms\models\route" || "docotel\dcms\models\assignment"))
                return $id;
            else
    ...
    public function leaveTrail($action, $name = null, $value = null, $old_value = null, $id =null, $model = null)
    {
        if($this->auditTrail === null) {
            ...
            $auditTrail->model = self::getLogModel($model);
            $auditTrail->action = $action;
            $auditTrail->model_id = self::getLogModelId($id, $auditTrail->model);
            $auditTrail->field = null;
            $auditTrail->stamp = $this->storeTimestamp ? time() : date($this->dateFormat); // If we are storing a timestamp lets get one else lets get the date
            $auditTrail->user_id = (string) $this->getUserId(); // Lets get the user id
            $auditTrail->user_name = !empty(Yii::$app->getUser()->identity->username) ? Yii::$app->getUser()->identity->username : 'Guest';
            $auditTrail->ip_address = $_SERVER['REMOTE_ADDR'];
            $auditTrail->url_referer = Yii::$app->request->url;
            $auditTrail->browser = $_SERVER['HTTP_USER_AGENT'];

            $auditTrail->save();
            $this->auditTrail = $auditTrail;
        }

        $id_audit_trail = $this->auditTrail->audit_trail_id;
        if ($this->active) {
            $log = new AuditTrailDetail();

            $log->audit_trail_id = $id_audit_trail;
