...
use docotel\dcms\models\form\Login;
use docotel\dcms\models\form\PasswordResetRequest;
use docotel\dcms\models\form\ResetPassword;
use docotel\dcms\models\form\Signup;
use docotel\dcms\models\form\ChangePassword;
use docotel\dcms\models\form\CreateUser;
use docotel\dcms\models\form\User as UserForm;
...
class UserController extends BaseController
{
    ...
    public function beforeAction($action)
    {
        if (parent::beforeAction($action)) {
            if (Yii::$app->has('mailer') && ($mailer = Yii::$app->getMailer()) instanceof BaseMailer) {
                ...
                $mailer->setViewPath('@docotel/dcms/mail');
            }
            return true;
        }
        return false;
        ...
        if (!empty($post)) {
            ...
            if ($model->validate()) {
                if ($this->userService->createAllUserGroup($post, $model)) {
                    ...
                    $model->addError('email', Yii::t('rbac-dcms', 'This email address has already been taken.'));
                }
            }
        }
    ...
    {
        $user = $this->loadModel('\docotel\dcms\models\User', $id);
        $model = new UserForm();
        $groups = $this->userService->getAllGroup();
        $listGroup = ArrayHelper::map($groups, 'group_id', 'name');
    ...
    public function actionView($id)
    {
        return $this->render('view', [
            'model' => $this->loadModel('\docotel\dcms\models\User', $id),
        ]);
    }
    ...
    {
        $this->loadModel('\docotel\dcms\models\User', $id)->delete();
    ...
    public function actionSignup()
    {
        ...
        if (!empty($post)) {
            ...
            if ($model->validate()) {
                ...
                return $this->redirect('/dcms/user/signup');
            }
        }
    ...
    public function actionResetPassword($token)
    {
        ...
        //split was deprecated in php 5.3.0
        $var_timereq = explode('_', $token);
        $time_request = end($var_timereq);
    ...
    public function actionActivate($id)
    {
        ...
        if ($user->status == User::STATUS_INACTIVE) {
            ...
            if ($user->save()) {
                Yii::$app->getSession()->setFlash('success', Yii::t('yii', 'Berhasil diaktivasi') );
                return $this->goHome();
            } else {
