    ...
    public function checkJson() {
        if($this->data) {
            ...
            if(json_last_error() !== JSON_ERROR_NONE) {
                $this->addError('data', 'format JSON tidak valid');
            }
        }
    }
        ...
        if (!Yii::$app->getAuthManager()->getRule($name)) {
            try {
                ...
                if ($rule instanceof \yii\rbac\Rule) {
                    ...
                    $this->addError('ruleName', Yii::t('rbac-dcms', 'Invalid rule "{value}"', ['value' => $name]));
                }
            } catch (\Exception $exc) {
                $this->addError('ruleName', Yii::t('rbac-dcms', 'Rule "{value}" does not exists', ['value' => $name]));
            }
        }
    }
